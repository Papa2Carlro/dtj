...
exports.parseTraceqlSubset = parseTraceqlSubset;
exports.formatTraceqlSubset = formatTraceqlSubset;
...
function tokenize(input) {
    ...
    if (s.length > constants_1.QUERY_TEXT_MAX_CHARS) {
        return fail(`query exceeds ${constants_1.QUERY_TEXT_MAX_CHARS} characters`);
    }
    if (s.includes("\0"))
        return fail("query contains NUL");
    if (s.includes(";"))
        return fail("multiple statements are not supported");
    const tokens = [];
    let i = 0;
    ...
    while (i < s.length) {
        ...
        if (/[0-9]/.test(c)) {
            ...
            if (!Number.isSafeInteger(n))
                return fail("integer out of safe range");
 ...
 */
function parseTraceqlSubset(input) {
    ...
    if (peekKw() === "WHERE") {
        ...
        for (;;) {
            ...
            if (!filterKey) {
                return fail(`unsupported field '${fieldTok.value}' (allowed: domain, category, event_name, severity, correlation)`);
            }
            const op = take();
    ...
    }
    {
        ...
        if (err)
            return fail("LIMIT is required in explorer TraceQL subset");
    }
    const limTok = take();
    if (limTok.kind !== "number")
        return fail("LIMIT must be an integer");
    if (limTok.value < 1 || limTok.value > constants_1.EVENTS_PAGE_MAX) {
        return fail(`LIMIT must be in 1..=${constants_1.EVENTS_PAGE_MAX}`);
    }
    if (peek().kind !== "eof") {
...
function formatTraceqlSubset(filters, limit) {
    const preds = [];
...
//# sourceMappingURL=traceqlSubset.js.map